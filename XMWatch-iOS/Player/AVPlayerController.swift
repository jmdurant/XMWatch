...
@MainActor
final class AVPlayerController: PlayerCoordinating {
    ...
    func logPlayerState() {
        guard let player else {
            writeDebug("[AVPlayer] logState: no player")
            return
        }
        ...
        writeDebug("[AVPlayer] state: itemStatus=\(status) rate=\(rate) timeCtrl=\(tcs) err=\(err) audioRoute=[\(outputs)]")
    }
        ...
        statusObservation = player.currentItem?.observe(\.status) { [weak self] item, _ in
            Task { @MainActor in
                ...
                case .readyToPlay:
                    ...
                    self?.onMediaLoaded?()
                case .failed:
                    writeDebug("[AVPlayer] FAILED: \(item.error?.localizedDescription ?? "unknown")")
