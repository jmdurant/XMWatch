...
final class XMRadioService {
    ...
    private func performPlayback(channel: XMChannel, generation: UUID) async -> Bool {
        ...
        guard playerController?.isSuspended != true else { return false }
        ...
        // The controller starts audio only after session activation succeeds.
        dbg("playing \(proxyURL.absoluteString)")
        controller.play(proxyURL)
        isPaused = false

        // Publish the selected channel while audio activation/loading completes.
